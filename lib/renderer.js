...
RequestContext.prototype.render = function(view) {
    if(view === undefined) {
        ...
        var self = this;
        template.fill(viewFile, this.model, this.encoding, function(err, content) {
            if(!err) {
                self.send(content);
            } else {
...
RequestContext.prototype.renderText = function(text) {
    ...
    if(!hasModel) {
        ...
        this.send(template.fillText(text, this.model));
    }
};
...
RequestContext.prototype.renderError = function(status, error) {
    ...
    fs.stat(viewFile, function(err, stats) {
        if(!err && stats.isFile()) {
            template.fill(viewFile, {error: error}, self.encoding, function(err, content) {
                if(err) {
                    self.handleError(err);
                } else {
                    self.send(content);
